    ...
    <div class="training">
        ...
        <div id="training-block">
          <div class="test-block" ref="scroller">
              <div class="slider">
				<div class="info" v-if="currentPage ==0">
					<div class="editors">
						<h1 v-if="currentPage == 0" >{{lastInfoTitle}}</h1>
						<hr/>
						<div v-if="currentPage == 0"  class="lastinfo" v-html="lastInfopage"></div>
						<div v-if="filteredQuestions.length == 0"  class="lastinfo" v-html="nolanguageinfo"></div>
					</div>
				</div>
				<div class="info" v-if="currentPage != 0">
...
//		Slide,
  ...
  data: function(){
    return{
		...
		trainingpage: this,
		currentPage: 0,
		currentSetting: 'vragen',
		sub: null
    }
  },
  methods:{
	getSubs: function(){
		HTTP.get('/training/single/'+this.$route.params.training+'/user/'+auth.getId())
		.then(response => {
			console.log(response.data)
			this.sub = response.data.training
			return response.data.training
		})
	},
	getTests: function(){
		console.log(this.sub)
		for(var item in this.sub){
			if(this.sub[item].isVisible == 1){				
				HTTP.get('/questions/'+this.sub[item].subonderdeel_id)
				.then(response => {
					console.log(response.data)
					this.questions = this.questions.concat(response.data.question)
	...
	nextSlide() {
		if(this.currentPage < this.filteredQuestions.length && this.currentPage != 0 && this.currentSetting == 'vragen'){
			this.$refs.indicators1[this.currentPage -1].classList.add('pageDone')
			this.$refs.scroller.scrollTop = 0
			this.currentPage = this.currentPage + 1
		}
		else if(this.currentPage == 0 && this.currentSetting == 'vragen'){
			this.currentPage = this.currentPage + 1
		}	
		else if(this.currentPage == this.filteredQuestions.length +1 && this.currentSetting == 'vragen'){
			//this.$refs.indicators1[this.currentPage -1].classList.add('pageDone')
			this.$refs.scroller.scrollTop = 0
			this.finished = true
		}	
 ...
 computed:{
	filteredQuestions: function(){
		...
		for(var item in this.questions){
			var index = this.sub.findIndex(p => p.subonderdeel_id == this.questions[item].subonderdeel_id)
			var dif = []
			if(this.sub[index] != null){
				dif = this.sub[index].difficulty.split('')
			}
			if(dif.length != 0){
